            ...
            process.nextTick(function () {
                ...
                if (!options.host || !options.user || !options.port || !options.dumpPath || !options.database) {
                    ...
                    fs.exists((extName === '') ? options.dumpPath : dirName, function (exists) {
                        if (!exists) {
                            error = new Error('dumpPath doesn\'t exist');
                            ...
                            if (options.password) {
                                ...
                                command = util.format('pg_dump -h %s -p %d --no-password -U %s -F c -b -v -f %s %s', options.host, options.port, options.user, filePath, options.database);
                            }
            ...
            process.nextTick(function () {
                ...
                if (!options.user || !options.host || !options.port || !options.sqlFilePath || !options.database) {
                    ...
                    fs.exists(options.sqlFilePath, function (exists) {
                        if (!exists) {
                            error = new Error('sqlFilePath doesn\'t exist');
                            error.name = 'InvalidFilePath';
                            callback(error, null);
                        } else {
                            if (options.password) {
                                command = util.format('pg_restore -h %s -p %d -W -U %s -d %s %s', options.host, options.port, options.user, options.database, options.sqlFilePath);
                            } else {
                                command = util.format('pg_restore -h %s -p %d --no-password -U %s -d %s %s', options.host, options.port, options.user, options.database, options.sqlFilePath);
                            }
                            ls = exec(command, function (error, stdout, stderr) {
                                if (error !== null) {
                                    callback(error, null, null);
